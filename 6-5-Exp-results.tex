...
Chapter \ref{sec:Approach} introduced four experiments to answer research questions of this thesis. They require synthetic datasets to examine the hypothesis of this thesis. However, we test experiment 1 and 3 with real datasets to test if this means is effective to assess a model fit with real data. The generation process of synthetic data with respect to each skills assessment models is given in details in previous chapter. In this chapter we introduce real and synthetic datasets to examine the hypothesis and discuss the results of each experiment:
...
There are two possibility for visualization of the predictive performance of seven models over a dataset, Figure \ref{Fig:Predictive-Preformace_rep} shows these two representations. We used both of them in this thesis for an easier understanding. Figure \ref{Predictive-Preformace_Base} shows the comparative performance signature which is the difference in accuracy of each model's performance from the expected value, which serves as a baseline. Figure \ref{Predictive-Preformace_Sig} shows the performance signature of seven models over a dataset.
...
Figure~\ref{figSampleSize} shows the results of extracting performance signature from datasets with different number of students. Obviously, the signature pattern did not change significantly except for IRT-based generated dataset that slightly changed the pattern for small sample size. Also decreasing the sample size downto less than 100 students can change the signature pattern, because in the learning phase, the training set of the model could not be learned properly.
