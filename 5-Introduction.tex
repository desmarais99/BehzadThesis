...
%\note{change: this term would refer to the \textit{performance vector} of a real data set; and refer to performance prototype for a model class vector.  can also define ``target \textit{performance vector}'' to refer to the data set to classify.}%target dataset that we want to classifye
...
Still the question of sensitivity of the ``performance signature'' to contextual factors should be considered in the comparison of the \textit{performance vectors}. The other contribution is to test the stability of the ``performance signature'' of synthetic datasets over different data specific parameters (such as sample size, average success rate, etc.) generated with the same underlying model. 
